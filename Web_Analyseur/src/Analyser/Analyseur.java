...
public class Analyseur {
				...
				while (matcher.find()){
					for (int i = 2; i <= Relation.patronNbrTerms.get(patron); i++) {
						...
						if (unique(type,matcher.group(1),patron,matcher.group(i),matcher.group())) {
							// Test d'ambiguitÈ et dÈsambiguation (par contraintes sÈmantiques)
	...
	private boolean unique(String type, String term1, String patron, String term2, String contexte) {
		/*
		 * MÈthode Èvitant la confusion entre patrons.
		 */
		for (ArrayList<String> Set : Relation.typePatrons.values()) {
			for (String pattern : Set) {
				...
				// Cas 2 : [(Terme1+Patron) ou (Patron+Terme2)] est un patron --> Interdire la crÈation d'une relation. 
	...
	private String desambiguation(String inputType, String patron, String term1, String term2) {
		 ...
		 * Utilise l'API de jeuxdemots pour vÈrifier les contraintes sur les termes. 
		 * 
		 * */
		String type = inputType;
		if (patron.equals("a des")) {
			// Simulation de contraintes sÈmantiques sur le patron "a des" (A titre d'exemple).
	...
	private boolean isAmbigu(String patron) {
		/*
		 * Liste de patrons qui crÈent une ambiguitÈ / prÍtent ‡ confusion.
		 */
	...
	public void parser(){
		...
		/*
		 * Probleme a rÈgler : le texte commence par null/n, essayer de l'enlver du texte
		 */
	...
	public boolean foundRelation(Relation relation){
		/*
		 * V√©rifie si une relation a d√©j√† √©t√© trouv√©e. 
		 */
	...
	public void displayResults(JspWriter out) throws IOException{
		/*
		 * Affiche la liste des relations trouv√©es. 
		 */
